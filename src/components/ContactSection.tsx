...
const ContactSection: React.FC<ContactSectionProps> = ({ darkMode }) => {
  ...
  // ✅ Corrected social links (GitHub fixed)
  const socialLinks = [
    { icon: Github, label: 'GitHub', href: 'https://github.com/Varshine04', color: 'hover:text-[#2ea44f]' },
    { icon: Linkedin, label: 'LinkedIn', href: 'https://www.linkedin.com/in/varshine12/', color: 'hover:text-blue-600' },
    { icon: Code, label: 'LeetCode', href: 'https://leetcode.com/VARSHINE_T/', color: 'hover:text-orange-500' }
  ];
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-12 max-w-6xl mx-auto">
          ...
          >
            ...
            <div className="space-y-4">
              <motion.a 
                ...
                whileTap={{ scale: 0.98 }} 
                className="w-full flex items-center justify-center space-x-3 py-4 px-6 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-300"
              >
                <Download size={20} />
          ...
          <motion.div 
            ...
            viewport={{ once: true }} 
            className={`p-8 rounded-2xl backdrop-blur-md shadow-2xl border ${darkMode ? 'bg-gray-800/50 border-gray-700/50' : 'bg-white/50 border-gray-200/50'}`}
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.6 }} viewport={{ once: true }}>
                ...
                <input 
                  ...
                  required 
                  className={`w-full px-4 py-3 rounded-lg backdrop-blur-md border transition-all duration-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent ${darkMode ? 'bg-gray-700/50 border-gray-600 text-white placeholder-gray-400' : 'bg-white/50 border-gray-300 text-gray-900 placeholder-gray-500'}`} 
                />
              </motion.div>
              {/* Email */}
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.8 }} viewport={{ once: true }}>
                ...
                <textarea 
                  ...
                  required 
                  className={`w-full px-4 py-3 rounded-lg backdrop-blur-md border transition-all duration-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none ${darkMode ? 'bg-gray-700/50 border-gray-600 text-white placeholder-gray-400' : 'bg-white/50 border-gray-300 text-gray-900 placeholder-gray-500'}`} 
                />
              </motion.div>
              <motion.button 
                type="submit" 
                ...
                whileHover={{ scale: 1.02 }} 
                whileTap={{ scale: 0.98 }} 
                className="w-full flex items-center justify-center space-x-3 py-3 px-6 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold rounded-lg shadow-lg hover:shadow-xl transition-all duration-300"
              >
                <Send size={18} />
